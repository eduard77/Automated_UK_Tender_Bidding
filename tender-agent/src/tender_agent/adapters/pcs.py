...
PCS publishes OCDS release packages. Default endpoint:
    ...
    async def fetch_since(self, since: datetime) -> AsyncIterator[NormalisedTender]:
        ...
        while url:
            ...
            try:
                ...
                logger.error("pcs.fetch_failed", error=str(exc), url=url)
                break
            ...
            for release in releases:
                try:
                    ...
                    logger.warning(
                        "pcs.normalise_failed", error=str(exc), ocid=release.get("ocid")
                    )
